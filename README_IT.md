...
### 1. Installazione diretta（consigliato）

Se Rust è già installato, puoi installare direttamente da crates.io ed eseguire:
...
| `j` | Cerca brani aggregati |
| `p` | Cerca playlist online |
| `i` | Visualizza informazioni brano |
| `a` | Richiedi brani consigliati |
| `Shift+A` | Consiglio playlist intelligente |
| `Shift+S` | Consiglio brani simili |
| `f` | Aggiungi/Rimuovi preferito |
| `v` | Visualizza preferiti |
| `m` | Visualizza directory musicale |
...
| `w` | Apri cronologia playlist intelligenti |
| `x` | Importa playlist M3U |
| `e` | Esporta playlist M3U |
| `u` | Entra in modalità calibrazione temporale dei testi |
| `q` | Esci dal programma musicale |


### Tasti di ricerca brani
...
| `s/n/j` | Cambia ricerca locale/online/aggregata |
| `Esc` | Esci dalla ricerca |

### Tasti lista preferiti
...
### Tasti consiglio playlist intelligente

| Tasto | Azione |
|------|------|
| Input di caratteri | Inserisci descrizione / tema della playlist intelligente |
| Preset | Clicca un tema preimpostato per un consiglio rapido |
| `Backspace` | Elimina carattere |
| `Enter` | Avvia generazione playlist consigliata / Riproduci brano selezionato |
| `↑/↓` | Seleziona brano consigliato |
| Rotellina mouse | Scorri l'elenco brani su/giù |
| `d` | Elimina il record corrente della cronologia playlist intelligente |
| `Esc` | Torna alla playlist |

### Tasti cronologia playlist intelligenti

| Tasto | Azione |
|------|------|
| `↑/↓` | Seleziona playlist intelligente storica |
| `Enter` | Riapri e riproduci la playlist storica selezionata |
| `PgUp/PgDn` | Pagina su/giù |
| `d` | Elimina il record storico selezionato |
| `Esc` | Torna alla playlist |

### Tasti riproduzioni recenti

| Tasto | Azione |
|------|------|
| `↑/↓` | Seleziona brano riprodotto di recente |
| `Enter` | Riproduci brano selezionato |
| `PgUp/PgDn` | Pagina su/giù |
| `d` | Elimina record di riproduzione recente |
| `Esc` | Torna alla playlist |
...
```text
src/
├── main.rs       # Punto di ingresso (parsing argomenti, inizializzazione, ripristino/salvataggio configurazione)
├── defs.rs       # Definizioni comuni (enumerazioni PlayMode/PlayState, strutture MusicFile/Playlist)
├── langs.rs      # Pacchetti lingua (gestione centralizzata testi traduzione in 11 lingue, accessore lingua globale)
├── audio.rs      # Controllo audio (wrapper rodio, riproduzione/pausa/ricerca/volume/progresso)
├── analyzer.rs   # Analizzatore audio (RMS in tempo reale, smoothing EMA, rendering forma d'onda)
├── playlist.rs   # Gestione playlist (scansione directory, caricamento parallelo durata, selezione cartella)
├── lyrics.rs     # Parsing testi (formato LRC, ricerca locale, rilevamento codifica, download in background)
├── search.rs     # Ricerca/download online (ricerca Kuwo + Kugou + NetEase, download, recupero commenti, query informazioni canzoni in streaming)
├── config.rs     # Gestione configurazione (serializzazione JSON, elementi configurazione persistenti)
├── desktop_lyrics.rs # Finestra flottante testi desktop (Windows API/processo figlio Linux, trasparenza/posizione/trascinamento/scorciatoie)
├── ui.rs         # Interfaccia utente (framework Ratatui, rendering terminale, gestione eventi, modalità multivista, sistema temi/lingue)
└── ui/
    ├── input.rs      # Gestione input
    ├── render.rs     # Logica rendering
    ├── layout.rs     # Gestione layout
    ├── theme.rs      # Sistema temi
    ├── mouse.rs      # Interazione mouse
    ├── terminal.rs   # Gestione terminale
    ├── format.rs     # Strumenti formattazione
    └── view_model.rs # Modello vista
```
...
## Versione 2.2.0 (2026-05-20)

### 🎉 Nuove funzionalità
- ✨ **Raccomandazione playlist intelligente**: premi `Shift+A` per entrare nella modalità di input della playlist intelligente. Dopo aver inserito una descrizione o cliccato un preset, il modello consiglia in streaming 20~30 brani a tema, aggiungendoli uno per uno alla lista delle raccomandazioni di playlist intelligente; la riproduzione inizia automaticamente non appena il primo brano viene analizzato. Le playlist intelligenti possono essere salvate come cronologia e riaperte con `W`.
- ✨ **Brani consigliati di oggi**: i brani consigliati di oggi ora vengono prodotti in streaming. Ogni brano restituito dal modello viene immediatamente aggiunto all'area in alto «Brani consigliati di oggi», quindi non è più necessario attendere il completamento della generazione di tutti i brani. Il suggerimento «Recupero dei brani consigliati in corso...» viene nascosto non appena compare il primo brano. Ogni volta vengono consigliati 5~10 brani e il risultato non contiene più motivi di raccomandazione o campi superflui.
- ✨ **Raccomandazione brani simili**: durante la riproduzione, premi `Shift+S` per far sì che il modello consigli in streaming 5~10 brani fortemente simili, basandosi su stile/genere, lingua, epoca, BPM, atmosfera/umore, caratteristiche armoniche/di arrangiamento, artisti/brani simili, ecc. del brano in riproduzione. Vengono mostrati nell'area in alto «Brani simili - Brano sorgente:». Fai clic o usa i tasti sinistra/destra per selezionare e premi Invio per scaricare e riprodurre.
- ✨ **Ottimizzazione delle raccomandazioni**: ora viene utilizzato un file di preferenze del comportamento di riproduzione più accurato (`preferences.json`) anziché basarsi solo sui dati della cronologia di riproduzione (`history.json`). Dopo il completamento del caricamento dei brani consigliati di oggi e dei brani simili, il primo brano non è più evidenziato per impostazione predefinita; la selezione inizia dal primo brano solo dopo che l'utente preme per la prima volta un tasto freccia o scorre il mouse. La barra superiore delle raccomandazioni passa in modo fluido tra lo stato di caricamento e lo stato con risultati, evitando che «Recupero dei brani consigliati in corso...» e i nomi dei brani vengano visualizzati contemporaneamente.

- ✨ **Miglioramento del comportamento del tasto `R`**: `R` ora attiva/disattiva i brani consigliati di oggi; attivandoli i brani consigliati di oggi vengono aggiornati automaticamente, mentre disattivandoli l'area in alto delle raccomandazioni viene chiusa direttamente, eliminando il comportamento precedente che consentiva solo l'attivazione.


### 🔧 Correzioni di bug
- 🐞 **Durata dei brani Apple Music mancante**: risolto il problema per cui, importando una playlist tramite una classifica preimpostata o un link Apple Music, solo il primo brano mostrava la durata mentre gli altri visualizzavano `--:--`. Apple Music ora chiama l'API `amp-api.music.apple.com` in base allo storefront nell'URL (ad es. `cn` / `hk` / `tw` / `us` / `kr` / `jp`), ottenendo in modo affidabile il valore `durationInMillis` di ogni brano.
- 🐞 **Sottotitolo errato durante l'importazione di playlist tramite URL**: risolto il problema per cui, dopo l'importazione di URL online non corrispondenti a classifiche preimpostate, il sottotitolo della ricerca della playlist mostrava «nome della sorgente + URL/parametri» invece del vero titolo della pagina.
- 🐞 **Invio non riproduce nella pagina dei risultati della playlist intelligente**: risolto il problema per cui, una volta completata la generazione della lista di raccomandazioni della playlist intelligente, lo spostamento del cursore e la pressione di Invio non riproducevano il brano selezionato e la riproduzione era possibile solo riaprendo la playlist dalla cronologia. È stato aggiunto un ramo Invio dedicato alla pagina dei risultati della playlist intelligente, che non viene più influenzato dallo stato `online_searching`.

---

## Versione 2.1.0 (2026-05-17)
...
#### Refattorizzazione UI con Ratatui
- ✨ **Aggiornamento framework UI**: Refattorizzazione del codice UI che usa direttamente crossterm per utilizzare il framework Ratatui, fornendo astrazioni TUI di livello superiore e una migliore organizzazione del codice
- ✨ **Refattorizzazione modulare**: Divisione di `ui.rs` in diversi sottomoduli: `input.rs`(gestione input), `render.rs`(logica rendering), `layout.rs`(gestione layout), `theme.rs`(sistema temi), `mouse.rs`(interazione mouse), `terminal.rs`(gestione terminale), `format.rs`(strumenti formattazione), `view_model.rs`(modello vista)
- ✨ **Ottimizzazione struttura codice**: Il codice UI è più modulare, manutenibile, con responsabilità funzionali chiaramente separate
...
- 🎨 **Miglioramento consistenza UI**: Ratatui fornisce componenti e un sistema di stili unificati, garantendo consistenza ed estensibilità degli elementi dell'interfaccia

### 💻 Dettagli tecnici

#### Aggiornamenti dipendenze
- ➕ Aggiunta dipendenza `ratatui`(versione 0.29, con funzione crossterm)
- ♻️ Conservazione di `crossterm` come libreria di controllo terminale di base

#### Aggiornamenti struttura progetto
- ♻️ Directory `ui/`: Aggiunta di diversi sottomoduli UI per disaccoppiamento funzionale e riutilizzo codice
...
- 🛠️ **Interfaccia incompleta al primo avvio su Linux**: corretto un problema per cui l'interfaccia era ridotta nell'angolo in alto a sinistra del terminale al primo avvio del programma su Linux e richiedeva un clic per essere visualizzata completamente. Aggiunta attesa di 50ms dopo l'ingresso nell'alternate screen, ri-query della dimensione del terminale e pulizia dello schermo
- 🛠️ **Nessun suggerimento per playlist vuota**: corretto un problema per cui la playlist era vuota senza indicazione al primo avvio senza directory musicale selezionata. Aggiunto suggerimento «Premi o per selezionare la directory musicale» (stesso stile del suggerimento nell'area dei testi)
- 🛠️ **Overflow dello sfondo blu della riga selezionata**: corretto un problema per cui lo sfondo blu della riga selezionata si estendeva oltre il limite del pannello sinistro nell'area dei testi. Sostituzione di `Clear(UntilNewLine)` con riempimento di spazi di larghezza esatta
- 🛠️ **Residuo dei testi precedenti nell'area dei testi**: corretto un problema per cui quando si passava a un brano senza testi, i testi del brano precedente rimanevano visibili. Pulizia di tutte le righe prima del disegno
- 🛠️ **Nessun ridisegno al ridimensionamento della finestra in pausa/fermo**: corretto un problema per cui l'interfaccia non si aggiornava immediatamente al ridimensionamento del terminale in stato di pausa o fermo. Aggiunta gestione dell'evento `Event::Resize`
- 🛠️ **Paginazione commenti non visibile in pausa**: corretto un problema per cui PageUp/PageDown in modalità commenti non venivano visualizzati in pausa o fermo. Aggiunto stato di caricamento commenti alla condizione di ridisegno periodico
- 🛠️ **Reimpostazione commenti al cambio brano in modalità commenti**: corretto un problema per cui i commenti venivano reimpostati al cambio brano in modalità commenti, perdendo la posizione di lettura corrente. Saltata la reimpostazione dei commenti in modalità commenti
- 🛠️ **Perdita di caratteri del titolo durante la riproduzione**: corretto un problema di perdita di caratteri nei titoli dei brani che iniziano con cifre/inglese (es: «17 anni» visualizzato come «1 anni»). Causa: i simboli Unicode `►★▶■❚` hanno larghezza ambigua nei terminali dell'Asia orientale (incoerenza di 1 o 2 colonne), causando spostamento del cursore e sovrascrittura dei caratteri successivi. Tutti i simboli Unicode ambigui sostituiti con caratteri ASCII di larghezza non ambigua: `►`→`>`, `★`→`*`, `▶`→`>>`, `■`→`||`, `❚`→`[]`

### 🔧 Miglioramenti

- 🎨 **Unificazione dei simboli UI in ASCII**: prefisso di riproduzione `>>` (in riproduzione), `||` (pausa), `[]` (fermo), marcatore di selezione `>`, marcatore preferiti `*`, marcatore directory corrente `>>`, marcatore evidenziazione testi `>`, marcatore selezione commento `>`, eliminazione dell'ambiguità di larghezza nei terminali dell'Asia orientale
- 📝 **Ottimizzazione del testo del suggerimento per playlist vuota**: modificato da «Nessuna directory musicale disponibile selezionata, modalità lista vuota attivata, premi o per aprire la directory musicale» a «Nessuna directory musicale disponibile, modalità playlist vuota attivata, premi o per aprire la directory musicale», formulazione più precisa e naturale
- 📂 **Impostazione directory predefinita in assenza di directory disponibile**: quando nessuna directory è disponibile, imposta automaticamente la directory musicale predefinita (USERPROFILE/ter-music-rust/music) e aggiungi alla cronologia delle directory musicali; durante il download di brani dalla ricerca online, utilizza la directory musicale predefinita invece della directory di lavoro corrente
