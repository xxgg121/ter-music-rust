...
- ✨ **Karaoke 말줄임표 최적화**：Karaoke 모드에서 왼쪽 위 줄의 두 번째 구절과 오른쪽 아래 줄의 두 번째 구절이 가사 영역을 초과하면 끝에 `...`를 표시하여 긴 구절이 창 경계를 넘지 않도록 합니다
- ✨ **둥근 모서리 데스크톱 가사 영역**：데스크톱 가사 배경 영역이 둥근 모서리 표시를 지원하며, 네 모서리를 투명하게 처리하고 가장자리 잔상을 정리해 더 부드러운 창 모양을 제공합니다
...
- 🛠️ **Linux 데스크톱 가사 강조 색상 수정**：Linux에서 가로 스크롤 및 Karaoke 모드의 현재 줄 강조가 세로 스크롤보다 어둡게 보이던 문제를 수정하여 세 모드의 현재 줄 강조 밝기를 통일했습니다
- 🛠️ **Linux 데스크톱 가사 하단 위치 수정**：Wayland 등 `_NET_WORKAREA`를 가져올 수 없는 환경에서 `PgDn` 후 데스크톱 가사가 작업 표시줄 위쪽이 아니라 화면 맨 아래에 붙는 문제를 수정했습니다
- 🛠️ **데스크톱 가사 둥근 모서리 투명도 수정**：둥근 모서리 바깥 픽셀에 배경색이 남아 네 꼭짓점에 직각 흔적이 보이던 문제를 수정하고, 투명 영역의 RGB를 함께 지우며 가장자리 색상을 감쇠합니다
- 🛠️ **중국어 입력기 단축키 수정**：중국어 입력기 상태에서 `。`, `，`, `【`, `】`, `［`, `］`, `‘`, `’` 등의 문장부호가 탐색되지 않거나 실수로 곡 전환을 일으키던 문제를 수정했습니다
...
- 🛠️ **Linux 최초 실행 시 화면 불완전**: Linux에서 처음 프로그램을 실행할 때 인터페이스가 터미널 왼쪽 상단에 축소되어 클릭해야 완전히 표시되는 문제를 수정. alternate screen 진입 후 50ms 대기하여 터미널 크기를 다시 쿼리하고 화면을 지움
- 🛠️ **빈 재생 목록에 힌트 없음**: 처음 실행 시 음악 디렉토리를 선택하지 않은 경우 재생 목록이 비어 있지만 안내가 없는 문제를 수정. "o를 눌러 음악 디렉토리 선택" 힌트 추가 (가사 영역 힌트와 동일한 스타일)
- 🛠️ **선택된 행 파란색 배경 넘침**: 선택된 행의 파란색 배경 하이라이트가 왼쪽 패널 경계를 넘어 오른쪽 가사 영역으로 확장되는 문제를 수정. `Clear(UntilNewLine)`을 정확한 너비의 공백 채우기로 교체
- 🛠️ **가사 영역 이전 내용 잔존**: 노래를 전환한 후 새 노래에 가사가 없는 경우 오른쪽 가사 영역에 이전 노래 가사가 남아 있는 문제를 수정. 그리기 전에 모든 행을 지움
- 🛠️ **창 크기 조정 시 일시정지/정지 상태에서 다시 그리지 않음**: 터미널 창 크기를 조정한 후 일시정지 또는 정지 상태에서 UI가 즉시 업데이트되지 않는 문제를 수정. `Event::Resize` 이벤트 처리 추가
- 🛠️ **댓글 페이징이 일시정지 시 표시되지 않음**: 댓글 보기에서 PageUp/PageDown 작업 후 일시정지 또는 정지 상태에서 UI가 업데이트되지 않는 문제를 수정. 댓글 로딩 상태를 주기적 다시 그리기 조건에 추가
- 🛠️ **댓글 모드에서 노래 변경 시 댓글 재설정**: 댓글 모드에서 노래가 변경될 때 댓글이 재설정되어 사용자가 보던 내용이 손실되는 문제를 수정. 댓글 모드에서 댓글 재설정 건너뜀
- 🛠️ **재생 시 제목 문자 손실**: 재생 상태에서 숫자/영어로 시작하는 노래 제목의 문자가 손실되는 문제를 수정 (예: "17세"가 "1세"로 표시). 근본 원인은 `►★▶■❚` 등 Unicode 기호가 동아시아 터미널에서 너비가 모호 (1열 또는 2열 불일치)하여 커서 오프셋으로 후속 문자가 덮어쓰되는 것. 모든 모호한 너비 Unicode 기호를 너비가 명확한 ASCII 문자로 교체: `►`→`>`, `★`→`*`, `▶`→`>>`, `■`→`||`, `❚`→`[]

### 🔧 개선

- 🎨 **UI 기호를 ASCII로 통일**: 재생 접두사 `>>` (재생), `||` (일시정지), `[]` (정지), 선택 마커 `>`, 즐겨찾기 마커 `*`, 디렉토리 현재 마커 `>>`, 가사 하이라이트 마커 `>`, 댓글 선택 마커 `>`, 동아시아 터미널 너비 모호성 제거
- 📝 **빈 재생 목록 힌트 문구 최적화**: "사용 가능한 음악 디렉토리가 선택되지 않았습니다. 빈 목록 모드에 진입했습니다. o를 눌러 음악 디렉토리를 여세요"를 "사용 가능한 음악 디렉토리가 없습니다. 빈 재생 목록 모드에 진입했습니다. o를 눌러 음악 디렉토리를 여세요"로 변경하여 더 정확하고 자연스러운 표현으로 개선
- 📂 **사용 가능한 디렉토리 없을 때 기본 디렉토리 설정**: 사용 가능한 디렉토리가 없을 때 기본 음악 디렉토리(USERPROFILE/ter-music-rust/music)를 자동 설정하고 음악 디렉토리 기록에 추가. 온라인 검색으로 곡을 다운로드할 때 현재 작업 디렉토리 대신 기본 음악 디렉토리 사용
