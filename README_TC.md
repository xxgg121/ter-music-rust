...
- ✨ **卡拉 OK 省略號最佳化**：卡拉 OK 模式左上行第二句、右下行第二句超出歌詞區域時會在結尾顯示 `...`，避免長句溢出視窗邊界
- ✨ **圓角桌面歌詞區域**：桌面歌詞背景區域支援圓角顯示，四角透明並處理邊緣殘留，視窗視覺更柔和
...
- 🛠️ **Linux 桌面歌詞高亮顏色修復**：修復 Linux 下橫向捲動、卡拉 OK 模式當前行高亮比垂直捲動模式偏暗的問題，統一三種模式的當前行高亮亮度
- 🛠️ **Linux 桌面歌詞底部定位修復**：修復 Wayland 等無法取得 `_NET_WORKAREA` 的環境中，按 `PgDn` 後桌面歌詞貼到螢幕底邊而不是工作列上邊緣的問題
- 🛠️ **桌面歌詞圓角透明修復**：修復圓角外側像素仍殘留背景色、四個頂點可見直角痕跡的問題，透明區域同步清零 RGB 並衰減邊緣顏色
- 🛠️ **中文輸入法快捷鍵修復**：修復中文輸入法狀態下 `。`、`，`、`【`、`】`、`［`、`］`、`‘`、`’` 等標點無法快進快退或誤觸發切歌的問題
