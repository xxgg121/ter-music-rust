...
一个简洁实用的终端音乐播放器，使用 Rust 实现，支持本地/网络歌曲搜索下载、自动下载歌词显示，主题切换等功能，支持Windows、Linux、macOS系统。
...
### 📂 目录管理
- **音乐目录选择**：按 `o` 打开文件夹选择对话框
- **打开目录历史**：按 `h` 查看所有打开过的目录，快速切换
...
### ⚙️ 配置持久化
配置保存在 `程序目录/config.json`，自动保存和恢复：
...
├── config.rs     # 配置文件管理（JSON 序列化、7 项配置持久化）
└── ui.rs         # 用户界面（终端渲染、事件处理、多视图模式、主题系统）
```
